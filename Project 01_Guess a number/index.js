...
welcome();
await sleep(1000);
let random_generated_number = Math.floor(Math.random() * 100);
let chances = 3;
// console.log(random_generated_number)
const rainbowTitle = chalkAnimation.rainbow('You will be given 3 chances to guess the correct number (range: 1-100) \n');
await sleep(1500);
rainbowTitle.stop();
for (let i = 0; i < chances; i++) {
    ...
    if (number.number === random_generated_number) {
        ...
        if (i + 1 === chances) {
            await sleep(1000);
            const rainbowTitle = chalkAnimation.radar('Game Over, You failed to guess the right number !!!');
            await sleep(2700);
            rainbowTitle.stop();
        }
    }
