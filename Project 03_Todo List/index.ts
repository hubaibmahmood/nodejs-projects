...
const current_working_directory: string = args[1].slice(0, -8)
...
const addFunction = () => {
    ...
    if (newTask){
        ...
        if (fileData !== "\n" &&  fileData !== ""){
            ...
            fs.writeFile(current_working_directory + "todo.txt", newTask, 
...
const deleteFunction = () => {
    ...
    const deletenum:number = Number(args[3])
...
const doneFunction = () => {
    ...
    if (deletenum){
        ...
        const fileData:string = fs.readFileSync(current_working_directory + "todo.txt").toString()
        ...
        if (deletenum <= data.length){
            ...
            let doneData:string = fs.readFileSync(current_working_directory + "done.txt").toString()
...
const reportFunction = () => {
    ...
    let todoData:string = fs.readFileSync(current_working_directory + "todo.txt").toString()
    let doneData:string = fs.readFileSync(current_working_directory + "done.txt").toString()

    let todo: string[] = todoData.split("\n")
    todo = todo.filter(function (value) { 
        return value !== '';  
    })

    let done: string[] = doneData.split("\n")
